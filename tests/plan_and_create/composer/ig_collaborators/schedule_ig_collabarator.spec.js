/* Test to schedule Instagram post with collaborator */
...
test('Send instagram message with collaborator', async ({ page }) => {
	...
	await test.step('Setup user & accounts', async () => {
		await addFixture.command('igb_collaborator_schedule', 'pro_user_composer', true, 300);
		igbProfile = getObjectByName(global.fixture, 'igb_collaborator_schedule').instagramBusiness.username;
	...
	await test.step('Login as pro user', async () => {
		await loginPage.signIn('igb_collaborator_schedule');
	});
	...
	await test.step('Schedule IGB message with collaborator', async () => {
		await composePage.schedule();
	});

	await test.step('Verify IGB scheduled message in planner preview pane', async () => {
		await expect(composePage.feCallOuts).not.toBeVisible();
		await plannerPage.verifyTextInPreviewPane(scheduleText);
		await expect(plannerPage.instagramPreviewHeaderName).toHaveText(`${igbProfile} and ${igbCollaborator}`);
		await expect(plannerPage.instagramCollaboratorsonSidePane).toHaveText(`${igbCollaborator}`);
	});
