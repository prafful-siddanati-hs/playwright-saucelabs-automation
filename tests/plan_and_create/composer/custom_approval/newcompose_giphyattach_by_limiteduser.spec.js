...
test('Approve gif message from planner list view', async ({ page }) => {
	...
	let accounts = {
		plan_create_facebookpage: ['caGifFB']
	};
	let orgName = 'CA_GIF_' + Math.floor(Math.random() * 10000);
	const scheduleText = `Schedule message with giphy for approvals ${Date.now()}`;
	const scheduleTime = addHours(new Date(), 2);

	await test.step('Setup test user and accounts', async () => {
		await setUpEnterpriseUser.setUpEnterpriseUser(orgName, 'ca_gif_admin', accounts);
		await createNewUser.command('caGif_limited_user');
		await addUserToNewOrg.command('caGif_limited_user', orgName);
		await updateSNPermissions.command('SN_LIMITED', 'caGifFB', 'caGif_limited_user');
		limitedUserMemberId = global.member[1].memberId;
	});

	await test.step('Login as an admin user', async () => {
		await loginPage.signInSkipOnboarding('ca_gif_admin');
	});

	await test.step('Schedule a facebook message with gif using limited user', async () => {
	...
	await test.step('Login as limited user', async () => {
		await loginPage.signInSkipOnboarding('caGif_limited_user');
	});
