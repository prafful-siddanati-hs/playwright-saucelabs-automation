...
test('Approve message with video fom planner list view', async ({ page }) => {
	...
	let accounts = {
		plan_create_facebookpage: ['caVideoFB']
	};
	let orgName = 'CA_VIDEO_' + Math.floor(Math.random() * 10000);
	const scheduleText = `Schedule message with video for approvals ${Date.now()}`;
	const scheduleTime = addHours(new Date(), 2);

	await test.step('Setup test user and accounts', async () => {
		await setUpEnterpriseUser.setUpEnterpriseUser(orgName, 'ca_video_admin', accounts);
		await createNewUser.command('caVideo_limited_user');
		await addUserToNewOrg.command('caVideo_limited_user', orgName);
		await updateSNPermissions.command('SN_LIMITED', 'caVideoFB', 'caVideo_limited_user');
		limitedUserMemberId = global.member[1].memberId;
	});

	await test.step('Login as an admin user', async () => {
		await loginPage.signInSkipOnboarding('ca_video_admin');
	});

	await test.step('Schedule a facebook message with video using limited user', async () => {
	...
	await test.step('Login as limited user', async () => {
		await loginPage.signInSkipOnboarding('caVideo_limited_user');
	});
