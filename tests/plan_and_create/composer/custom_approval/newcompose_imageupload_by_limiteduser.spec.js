...
test('Approve message with image fom planner list view', async ({ page }) => {
	...
	let accounts = {
		plan_create_facebookpage: ['caImageFB']
	};
	let orgName = 'CA_IMAGE_' + Math.floor(Math.random() * 10000);
	const scheduleText = `Schedule message with image for approvals ${Date.now()}`;
	const scheduleTime = addHours(new Date(), 2);

	await test.step('Setup test user and accounts', async () => {
		await setUpEnterpriseUser.setUpEnterpriseUser(orgName, 'ca_image_admin', accounts);
		await createNewUser.command('caImage_limited_user');
		await addUserToNewOrg.command('caImage_limited_user', orgName);
		await updateSNPermissions.command('SN_LIMITED', 'caImageFB', 'caImage_limited_user');
		limitedUserMemberId = global.member[1].memberId;
	});

	await test.step('Login as an admin user', async () => {
		await loginPage.signInSkipOnboarding('ca_image_admin');
	});

	await test.step('Schedule a facebook message with image using limited user', async () => {
	...
	await test.step('Verify scheduled image message is present on planner list view', async () => {
	...
	await test.step('Login as limited user', async () => {
		await loginPage.signInSkipOnboarding('caImage_limited_user');
	});
