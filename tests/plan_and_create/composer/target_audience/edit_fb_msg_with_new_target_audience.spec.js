...
test('Verify new target audience value is added to facebook scheduled message', async ({ page }) => {
	...
	await test.step('Setup user & account', async () => {
		await addFixture.command('edit_fb_target_audience', 'pro_user_composer', true, 300);
		memberId = global.member[0].memberId;
	});

	await test.step('Login as pro user', async () => {
		await loginPage.signIn('edit_fb_target_audience');
	});
	...
	await test.step('Schedule facebook message with target audience', async () => {
		await createScheduleMessage.command(
			parseInt(memberId, 10),
			{
				messages: [
					{
						socialProfileId: parseInt(getObjectByName(global.fixture, 'edit_fb_target_audience').facebookPage.id),
						text: scheduleText,
						scheduledSendTime: formatISO(scheduleTime),
						targeting: {facebookPage: {ageMin: 18}}
					}
				]
			}
		);
	...
	await test.step('Add new FB target audience', async () => {
		...
		await expect(composePage.appliedTargetValue).toContainText('Ages: 18+; Countries: Canada;');
	});
	...
	await test.step('Verify updated message in planner preview pane', async () => {
		await plannerPage.verifyTextInPreviewPane(scheduleText);
		await expect(plannerPage.tagContainerText).toHaveText('Minimum age: 18; Countries: Canada;');
	});
