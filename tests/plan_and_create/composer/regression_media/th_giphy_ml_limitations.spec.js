...
test('Validate the media library\'s mixed media limits for Threads', async ({page}) => {
	...
	await test.step('Setup user & accounts', async () => {
		await addFixture.command('th_gif_limit', 'linkedin_enterprise', true, 300);
	});

	await test.step('Login as pro user', async () => {
		await loginPage.signInAsProUser('th_gif_limit');
	});
	...
	await test.step('Write a message', async () => {
		...
		await composePage.verifyThreadsPreview(mediaText);
	});
	...
	await test.step('Attach 2 more images from media library', async () => {
		await composePage.openMediaLibrary();
