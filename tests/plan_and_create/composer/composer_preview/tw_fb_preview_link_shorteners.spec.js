//This test is to validate the twitter and facebook page preview for link settings shorteners
...
test('Twitter and facebook page preview validations for link and existing link shortener', async ({ page }) => {
	...
	await test.step('Setup user & accounts', async () => {
		await addFixture.command('pw_tw_fb_preview_shortener', 'linkedin_enterprise', true, 300);
	});

	await test.step('Login as pro test user', async () => {
		await loginPage.signInAsProUser('pw_tw_fb_preview_shortener');
	});
	...
	await test.step('Select twitter and facebook accounts from profile picker', async () => {
	...
	await test.step('Verify applied presets on composer preview', async () => {
		...
		await expect(composePage.facebookLinkPrevewMedia, 'Link preview media is visible on composer preview').toBeVisible();
	});
