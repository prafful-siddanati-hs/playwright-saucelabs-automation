...
test('Approve send now and scheduled message from manage approvals view', async ({ page }) => {
	...
	let accounts = {
		plan_create_facebookpage: ['postNowScheduleToFB']
	};
	let orgName = 'Manage_approvals_' + Math.floor(Math.random() * 10000);
	const postNowText = `Post now message to approve ${Date.now()}`;
	const scheduleText = `Schedule message to approve ${Date.now()}`;
	const scheduleTime = addHours(new Date(), 2);

	await test.step('Setup test user and accounts', async () => {
		await setUpEnterpriseUser.setUpEnterpriseUser(orgName,'postNowSchedule_admin', accounts);
		...
		await updateSNPermissions.command('SN_LIMITED', 'postNowScheduleToFB', 'postNowSchedule_limited_user');
		limitedUserMemberId = global.member[1].memberId;
	});

	await test.step('Login as an admin user', async () => {
		await loginPage.signInSkipOnboarding('postNowSchedule_admin');
	});

	await test.step('Create a post now message for facebook using limited user', async () => {
	...
	await test.step('Schedule a facebook message using limited user', async () => {
		await createScheduleMessage.command(
			parseInt(limitedUserMemberId, 10),
			{
				messages: [
	...
	await test.step('Approve the post now message', async () => {
		await plannerPage.approveFromApprovalsListView(postNowText, 'Post');
	...
	await test.step('Approve the scheduled post', async () => {
		await plannerPage.approveFromApprovalsListView(scheduleText, 'Post');
		await expect(plannerPage.feCallOut).toBeVisible();
	});
